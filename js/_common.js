...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		if (isMobile()) {
			...
			$('.smooth-scroll main, .has-parallax, nav, .nmp-slider-wrapper:not(.content-slider), .showcase-lists .nmp-sync-slider, .next-project-image-wrapper, .slider-fixed-content').css({'height': winHeight});
		...
		$('.home-arrow').on('click', function() {	
			...
			if ($("body").hasClass("smooth-scroll")) {
				gsap.to(scrollbar, {duration: 1.5, scrollTop:heroTitleheight, ease:Power4.easeInOut});
				gsap.to('#ball', {duration: 0.3, borderWidth: '4px', scale:0.5, borderColor:'#999999', delay:0.15});
			} else {
		...
		gsap.utils.toArray('.carousel-shortcode-pin').forEach((carouselShortcodePin) => {
			...
			const clapatItems = carouselShortcodePin.querySelectorAll('.carousel-shortcode-thumbs .nmp-item');
			...
			if (!isMobile()) {	
				...
				$(".carousel-shortcode-thumbs .nmp-item .slide-inner").on('mouseenter', function() {
					$('#ball p').remove();
					var $this = $(this);			
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			if ($("#itemsWrapper").hasClass("no-fitthumbs")) {
				...
				$('#itemsWrapperLinks .trigger-item-link').on('click', function() {
				  	...
				  	const fadeOutTargets = $('#hero, #show-filters, .nmp-slider, .item, .item-caption-wrapper, #page-nav, footer, .fadeout-element');				  	
				  	const ball = $('#ball');
...
---------------------------------------------------*/
	...
	window.Sliders = function() { 
		...
		if( $('.content-slider').length > 0 ){
			...
			$('.content-slider.looped-carousel .nmp-slider, .content-slider.small-looped-carousel .nmp-slider').on('mouseenter mousemove', function() {	
...
---------------------------------------------------*/
	...
	window.Core = function() {
		...
		if (!isMobile() && !$('body').hasClass("disable-cursor")) {
			...
			//Blog Hover Effects			
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .nmp-sidebar-widget a").mouseenter(function(e) {	
				gsap.to('#ball', {duration: 0.2, borderWidth: '1px', scale: 1, opacity:0});
			});	
					
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .nmp-sidebar-widget a").mouseleave(function(e) {
				gsap.to('#ball', {duration: 0.3, borderWidth: '4px', scale:0.5, opacity:1});
			});
		}
